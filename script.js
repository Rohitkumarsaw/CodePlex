...
function renderCategories() {
  ...
  container.innerHTML = allChip + cats.map(([cat, count]) => {
    const catObj = (typeof categories !== 'undefined')
    ...
    return `
      <button class="category-chip${isActive ? ' active' : ''}" 
        data-category="${escHtml(cat)}" role="listitem" aria-pressed="${isActive}">
...
function verifySerialKey(productId) {
  ...
  if (!enteredKey) {
    ...
    if (errorMsg) { errorMsg.textContent = '⚠️ Please enter a serial key.'; errorMsg.classList.add('visible'); }
    return;
  }
  ...
  if (validKeys.includes(enteredKey)) {
    ...
    input.classList.add('error');
    input.value = '';
    if (errorMsg) { errorMsg.textContent = '❌ Invalid serial key. Please check and try again.'; errorMsg.classList.add('visible'); }
...
function saveProduct() {
  ...
  const productData = {
    ...
    previewImages: [document.getElementById('form-image').value.trim()],
    ...
    createdAt: id ? (products.find(p => p.id === id)?.createdAt || new Date().toISOString().split('T')[0]) : new Date().toISOString().split('T')[0],
    updatedAt: new Date().toISOString().split('T')[0],
    rating: id ? (products.find(p => p.id === id)?.rating || null) : null,
...
function getListItems(containerId) {
  const container = document.getElementById(containerId);
  if (!container) return [];
  return [...container.querySelectorAll('.dynamic-item-text')].map(el => el.textContent.trim()).filter(Boolean);
}

function getDownloadItems() {
  const container = document.getElementById('downloads-items');
  if (!container) return [];
  return [...container.querySelectorAll('.download-item-row')].map(row => ({
...
function setupEventListeners() {
  ...
  // Nav search toggle (scroll to controls)
  ...
  // Product cards (event delegation)
  const delegateProductAction = (containerId) => {
    document.getElementById(containerId)?.addEventListener('click', e => {
      ...
      if (viewBtn) { e.stopPropagation(); openProductModal(viewBtn.dataset.id); }
      else if (editBtn) { e.stopPropagation(); openProductForm('edit', editBtn.dataset.id); }
      else if (deleteBtn) { e.stopPropagation(); confirmDeleteProduct(deleteBtn.dataset.id); }
  ...
  document.getElementById('add-tag-btn')?.addEventListener('click', () => {
    ...
    if (input?.value.trim()) { addDynamicTag(input.value.trim()); input.value = ''; }
  });
  ...
  document.getElementById('add-feature-btn')?.addEventListener('click', () => {
    ...
    if (input?.value.trim()) { addDynamicFeature(input.value.trim()); input.value = ''; }
  });
  document.getElementById('feature-input')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') { e.preventDefault(); document.getElementById('add-feature-btn')?.click(); }
  });
  ...
  document.getElementById('add-key-btn')?.addEventListener('click', () => {
    ...
    if (input?.value.trim()) { addDynamicKey(input.value.trim()); input.value = ''; }
  });
  document.getElementById('key-input')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') { e.preventDefault(); document.getElementById('add-key-btn')?.click(); }
  });
  ...
  // Scroll: sticky header style & back to top
  window.addEventListener('scroll', () => {
